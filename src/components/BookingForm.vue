				...
				<v-card-text>
					<v-form ref="form">
						<v-row>
							...
							<v-col cols="6">
								<v-select label="Espacio comun" v-model="booking.common_space"
									:items="building.common_spaces" item-title="name" item-value="name" required>
								</v-select>
							</v-col>
						</v-row>
						<v-row>
							<v-col cols="12" sm="6" md="6">
								<v-text-field v-model="booking.date" label="Fecha" persistent-hint type="date" required>
								</v-text-field>
							</v-col>
							<v-col cols="12" sm="6">
								<v-select v-model="booking.shift" :items="shifts" item-title="title" item-value="key"

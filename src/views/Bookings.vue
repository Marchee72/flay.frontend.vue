<template>
	...
	<v-window v-model="tab">
		<v-window-item key="building-bookings" value="building-bookings" @click="fetchBuildingBookings">
			<v-container fluid>
				<v-row>
...
export default defineComponent({
	...
	methods: {
		async fetchBuildingBookings() {
			...
			var params = new Map<string, string>([[":building_id", this.userInfo.building?.id!]]);
			await fetch(null, params).then(_ => this.buildingBookings = data.value!.bookings)
		},
		async fetchUserBookings() {
			...
			var params = new Map<string, string>([[":user_id", this.userInfo.user?.id!]]);
			await fetch(null, params).then(_ => this.userBookings = data.value!.bookings)
		}
	}
});
</script>
